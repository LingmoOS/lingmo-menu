...
{
    Q_GADGET
    Q_PROPERTY(LingmoMenu::LabelItem::Type type READ type)
...
class LabelBottle : public QObject
{
    ...
    Q_PROPERTY(QList<LingmoMenu::LabelItem> labels READ labels NOTIFY labelsChanged)
public:
    explicit LabelBottle(QObject *parent = nullptr);

    int column() const;
    QList<LingmoMenu::LabelItem> labels() const;

    void setColumn(int column);
    void setLabels(const QList<LingmoMenu::LabelItem> &labels);
...
Q_DECLARE_METATYPE(LingmoMenu::AppListPluginGroup::Group)
Q_DECLARE_METATYPE(LingmoMenu::LabelItem)
Q_DECLARE_METATYPE(LingmoMenu::LabelBottle*)
